...
describe('RepairModel', () => {
  ...
  describe('findByVehicleId', () => {
    it('should return repairs for a specific vehicle', async () => {
      const mockRows = [
        {
          id: 'repair-1',
          vehicle_id: 'vehicle-1',
      ...
      const result = await RepairModel.findByVehicleId('vehicle-1');

      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM repairs WHERE vehicle_id = $1 ORDER BY created_at DESC'),
        ['vehicle-1']
      );
      expect(result).toHaveLength(2);
      expect(result[0].description).toBe('Oil change');
      expect(result[1].description).toBe('Brake replacement');
    });

    it('should return empty array when no repairs found', async () => {
      mockQuery.mockResolvedValue({ rows: [] });

      const result = await RepairModel.findByVehicleId('vehicle-2');
  ...
  describe('findByCustomerId', () => {
    it('should return repairs for a specific customer', async () => {
      const mockRows = [
        {
          id: 'repair-1',
          vehicle_id: 'vehicle-1',
      ...
      const result = await RepairModel.findByCustomerId('customer-1');

      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM repairs WHERE customer_id = $1 ORDER BY created_at DESC'),
        ['customer-1']
      );
    ...
    it('should return empty array when no repairs found', async () => {
      ...
      const result = await RepairModel.findByCustomerId('customer-2');
